...
const utils = {
    ...
    previewImage(list) {
		...
		buildCategoryTree(categoriesData) {
		    // Step 1: 创建 id → 完整分类对象的映射
		    const categoryMap = {};
		    categoriesData.forEach(category => {
		        categoryMap[category.metadata.name] = category;
		    });
		
		    // Step 2: 为每个分类添加 spec.childrenCategories 字段
		    categoriesData.forEach(category => {
		        // 初始化 childrenCategories 字段
		        if (!category.spec.childrenCategories) {
		            category.spec.childrenCategories = [];
		        }
		
		        // 填充子分类数据
		        category.spec.children.forEach(childId => {
		            const childCategory = categoryMap[childId];
		            if (childCategory) {
		                category.spec.childrenCategories.push(childCategory);
		            }
		        });
		    });
		
		    // Step 3: 返回顶级分类（未被其他分类引用的分类）
		    ...
		    const topLevelCategories = categoriesData.filter(category => {
		        return !allChildIds.has(category.metadata.name);
		    });
		
		    return topLevelCategories;
		},
		//查找某一分类下的所有子分类
		findCategorySubtree(tree, targetId) {
		    // 遍历树中的每一个分类
		    for (const category of tree) {
		        // 如果找到目标分类，直接返回它的完整结构（包括它的childrenCategories）
		        if (category.metadata.name === targetId) {
		            return category;
		        }
		        // 如果当前分类有子分类，递归查找
		        if (category.spec.childrenCategories && category.spec.childrenCategories.length > 0) {
		            const found = this.findCategorySubtree(category.spec.childrenCategories, targetId);
		            if (found) return found; // 找到则返回
		        }
		    }
		    return null; // 未找到
		}
