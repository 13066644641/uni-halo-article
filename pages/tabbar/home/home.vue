        ...
        <block v-else>
            ...
            <block v-if="calcIsShowCategory">
                <view class="flex flex-between mt-16 mb-24 pl-24 pr-24">
                    <view class="page-item_title text-weight-b ">精品栏目</view>
                ...
                <scroll-view class="category" scroll-x="true">
                    ...
                    <block v-else>
                        <view class="content" v-for="(category, index) in categoryList" :key="category.metadata.name"
                              @click="category.spec.childrenCategories && category.spec.childrenCategories.length == 0 ? 
															fnToCategoryBy(category) : fnToCategoryPage(category)">
...
export default {
    ...
    methods: {
                ...
                .then(res => {
                    this.categoryList = res.items.sort((a, b) => {
                        return b.postCount - a.postCount;
										...
										//组成树结构
										this.categoryList = this.$utils.buildCategoryTree(this.categoryList)
        ...
        fnToCategoryPage(category) {
						//uni.setStorageSync('categoryId', category ? category.metadata.name : '');
						// uni.switchTab({
						// 		url: '/pages/tabbar/category/category-detail'
						// });
						uni.navigateTo({
							url: '/pages/tabbar/category/category-detail?categoryId=' + (category.metadata ? category.metadata.name : '')
						});
        },
        // 所有的文章列表页面
        fnToArticlesPage() {
