            ...
            <block v-else>
                <tm-translate v-for="(item, index) in dataList" :key="index"
                    ...
                    <view class="catgory-card" :style="{backgroundImage:`url(${item.spec.cover})`}">
                        <view v-if="item.spec.childrenCategories && item.spec.childrenCategories == 0" class="content" @click="handleToCategory(item)">
                            ...
                            <view v-if="!haloConfigs.basicConfig.auditModeEnabled" style="font-size: 24rpx;color: #ffffff;margin-top: 6rpx;">
                                共 {{ item.postCount ? item.postCount : 0 }} 篇文章
                            </view>
                            <view v-else style="font-size: 24rpx;color: #ffffff;margin-top: 6rpx;">
                                共 {{ item.postCount ? item.postCount : 0 }} 篇内容
                            </view>
                        </view>
												<view v-else class="content" @click="fnToCategoryPage(item)">
												    <view style="font-size: 32rpx;color: #ffffff;">{{ item.spec.displayName }}</view>
												    <view style="font-size: 24rpx;color: #ffffff;margin-top: 6rpx;">
												        共 {{ item.spec.childrenCategories && item.spec.childrenCategories.length > 0 ? item.spec.childrenCategories.length : 0 }} 个分类
												    </view>
												</view>
                    </view>
                </tm-translate>
...
export default {
    ...
    methods: {
				fnToCategoryPage(category){
						//this.categoryId = category ? category.metadata.name : '';
						uni.navigateTo({
							url: '/pages/tabbar/category/category-detail?categoryId=' + (category ? category.metadata.name : '')
						});
				},
        fnGetData() {
            if (this.haloConfigs.basicConfig.auditModeEnabled) {
                ...
                .then(res => {
                    ...
                    if (this.isLoadMore) {
                        ...
                        this.dataList = tempItems;
										...
										//组成树结构
										console.log(this.$utils.buildCategoryTree)
										console.log(this.$utils.findCategorySubtree)
										this.dataList = this.$utils.buildCategoryTree(this.dataList);
										//查找某一分类下的子分类
										if(this.categoryId){
												const result = this.$utils.findCategorySubtree(this.dataList, this.categoryId)
												if(result){
													this.dataList = result.spec.childrenCategories;
												}
										}
										console.log(this.dataList)
                })
